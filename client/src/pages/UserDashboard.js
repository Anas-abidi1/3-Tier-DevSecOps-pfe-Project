...
// NOTE: this component makes requests with no explicit Authorization header
// (unlike Dashboard.js, which sets one manually). That only works correctly
// if ../axios.js has a request interceptor that attaches
// `Authorization: Bearer <token>` to every request automatically. I haven't
// seen axios.js yet, so I can't confirm that's actually happening — if it
// isn't, every request here silently 401s. Please share axios.js so this can
// be verified/fixed.
